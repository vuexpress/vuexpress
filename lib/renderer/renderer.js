...
class Renderer extends EventEmitter {
  ...
  getVueClass() {
    ...
    return Promise.all(needCompiledPlugins.map(pluginPath => this.compiler.import(pluginPath))).then(plugins => {
  ...
  getComponent(path, context) {
    return Promise.all([this.getVueClass(), this.compiler.import(path).then(object => object.default || object)]).then(([VueClass, VueOptions]) => {
